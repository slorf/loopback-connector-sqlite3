...
SQLite3.prototype.getInsertedId = function(model, info) {
  ...
  var idInjection = settings && settings.idInjection;
...
SQLite3.prototype.buildColumnDefinitions = function(model) {
  ...
  for (var propertyName in properties) {
    ...
    var pkDef = this._buildPKDefinition(model, propertyName);
    if (pkDef) {
      if (pkDef instanceof Error) return pkDef;
      pk[pkDef.id] = pkDef.columnName;
    }
  }
  if (pk.length) {
    line.push('PRIMARY KEY (' + pk.filter(function(val) { return val; }).join(',') + ')');
  }
  return line.join(',');
};
...
SQLite3.prototype._buildPKDefinition = function(model, propertyName) {
  var property = this.getModelDefinition(model).properties[propertyName];
  ...
  if (property.id === true) {
    return {id: 1, columnName: '"' + columnName + '"'};
  }
  if (property.id) {
    return {id: property.id, columnName: '"' + columnName + '"'};
  }
};
...
SQLite3.prototype.fromColumnValue = function(property, value) {
  if (value === null || value === undefined || !property) {
    return value;
  }
  switch (property.type.name) {
    case 'Number':
